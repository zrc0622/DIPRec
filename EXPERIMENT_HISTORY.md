...
## 14. 五组 reference / KL / 学习率优化（部分完成）

用户明确不做向量相似度实验，因此本轮取代13.3节的后续方向。脚本
`scripts/run_rl_optimization_sweep.py`已实现；2026-09-09收到A/B完整结果，
C因磁盘不足中断，D/E未开始。结果见14.1节。
...
全部完成后包含15阶段行（当前6行）、SFT差值、B−A/C−A/D−C/D−B/E−A比较、三区间各任务命中组率和KL。
...
CPU DDP快照/同步检查。shell语法、Python AST和空白检查通过。实现阶段尚未运行
正式四GPU训练；随后上传的效果结果见下节。实现检查日志保存在本地忽略目录
`analysis/rl_optimization_implementation_20260908/`。

### 14.1 2026-09-09 上传结果：A/B完成，C磁盘不足中断

本批zip的55个相关文件与工作树逐字节一致。A/B各完成3,455步及三个阶段评测；
预测对应相同4,866条Valid、1,795个用户，完成标记、配置、冻结探针哈希均通过校验。
新评测SFT与旧SFT的四项排名指标、目标和全部候选列表一致。无test调参。
同阶段A/B除reference及其允许字段外配置相同，仍为原exact+rank奖励、四任务。

C日志在`60/3455`附近出现`OSError: [Errno 28] No space left on device`，
诊断只有59条完整记录，第60行截断。`state.json`仍为training，`state.json.tmp`
为空，不能据此判断C仍正常训练。D/E尚未开始，低β和高LR均无可用效果结论。

| 模型/步数 | Recall@5 | NDCG@5 | Recall@10 | NDCG@10 |
|---|---:|---:|---:|---:|
| 初始SFT | .210234 | .181799 | .235923 | .190004 |
| A / 1,000 | .207768 | .181007 | .234895 | .189660 |
| A / 2,000 | .207357 | .181048 | .233457 | .189432 |
| A / 3,455 | .207974 | .181397 | .234073 | .189760 |
| B / 1,000 | .207357 | .181219 | .234073 | .189768 |
| B / 2,000 | .207974 | .180990 | .235512 | .189701 |
| B / 3,455 | .208796 | .181705 | .232840 | .189376 |

六个阶段均未超过SFT的四项核心指标。最终A新增14/丢失23个Top10命中，净少9；
B新增15/丢失30个，净少15。10,000次用户聚类配对bootstrap：A−SFT的
ΔR10=−.001850，95% CI [−.004414,+.000628]；B−SFT为−.003083
[−.005937,−.000402]。B−A为−.001233 [−.003067,+.000427]，尚不能认定
sync相对fixed有可靠差异。三项对比的ΔN10区间均跨零。上述为逐项区间，未做
多重比较校正，不覆盖训练运行间方差；本次A也不能用之前短跑对照的数值替代。

sync确实生效：A/B前512条训练记录完全相同，第513步首次出现KL/梯度/loss差异。
后段2,001–3,455步平均训练KL为A .006460、B .000772，但主任务命中组率为
40.82%/40.87%，几乎不变。B的KL参照物随policy移动，不能直接理解成相对初始
SFT的变化更小；valid六候选子集KL最终为A .008440、B .008893，也未更小。
该探针KL不是全策略KL。两组每步梯度范数均非零，生成长度4、截断率0。

固定主任务探针（train/valid各128条）更能解释当前现象。相对初始SFT：

| 切分 | 组 | Δ目标logp | Δ最高错误候选logp | Δmargin |
|---|---|---:|---:|---:|
| train | A | +.0113 | +.1184 | −.1071 |
| train | B | +.0116 | +.1189 | −.1073 |
| valid | A | −.0754 | +.1631 | −.2385 |
| valid | B | −.0704 | +.1730 | −.2434 |

margin为目标与五个固定错误候选中最高分的logp差，四个margin变化的用户聚类
95%区间均低于0。train也没有改善目标相对错误候选的优势，不能只解释成valid
过拟合。valid中初始SFT已命中的31条平均margin上升（A +.0864/B +.0620），
未命中的97条下降（A −.3424/B −.3410）。这是小样本的事后描述性诊断，支持
“已有优势被强化、未命中目标没有被有效拉上来”的现象，不证明唯一训练根因。
全任务零方差组仍约62%–63%，奖励稀疏可能参与其中，但不能据此认定唯一首因；
β过大也尚待C/D验证。此批不支持单独sync或仅延长相同训练能解决问题。

下一步先释放训练机器磁盘，再按原命令`--resume`。保留初始SFT及A/B的final、
step_1000、step_2000权重和评测文件，runner会校验并复用；C换新attempt从SFT
重跑，然后D/E，不从第60步恢复。剩余实验配置保持不变，分别用C−A、D−C/D−B、
E−A检查低β、sync交互和高LR是否同时改善margin与全量排名，再决定后续奖励方向。

重算脚本、SHA256、逐阶段统计和中文报告位于本地忽略目录
`analysis/rl_optimization_results_20260909/`。本次未修改训练/SFT代码或上传产物。

### 14.2 额外F组：4–7卡并行诊断（已实现，待运行）

用户授权利用4、5、6、7卡额外运行一组。新增独立入口
`scripts/run_rl_extra_experiment.py`：F设β=0、LR2e-6，关闭KL惩罚，其他保持A的
原exact+rank、四任务、同一SFT、G16/batch256、1epoch/3,455步及1,000/2,000
快照。用于与A(.01)、C(.001)比较，检查取消KL约束是否改善目标margin及排名；
不预设有效，不添加向量相似度或改变SFT。

```bash
python3 scripts/run_rl_extra_experiment.py --gpus 4,5,6,7
```

先释放训练文件系统的磁盘空间。默认独立端口29517、独立sweep目录
`rl_opt_extra_v1`，原`rl_opt_v1`状态保持只读。启动前核对输入/SFT哈希并复制原
冻结探针和SFT基线；不重新抽样，训练跳过共享数据预处理。评测只使用4号物理卡。
新增端口参数仅通过`DIPREC_MAIN_PROCESS_PORT`显式启用，原入口默认行为保持。

F中断后可加`--resume`：训练不完整则新attempt从SFT重跑，训练完整则补评测。
`--summarize_only`无需权重，生成F−SFT三阶段结果及已有A/C对应阶段对比；
C尚未完成则暂不比较，之后重新汇总补齐。β=0时TRL不计算reference KL，汇总
显式写null和`training_kl_available=false`，不误报为零；冻结六候选SFT KL继续测量。

本地检查覆盖原五组续跑、F独立目录/GPU/端口、冻结探针一致、源结果只读、
训练失败新attempt、评测失败复用训练、无KL日志汇总及shell默认端口兼容。
41项检查通过，包含C稍后完成时补充F−C对比；shell语法、dry-run及空白检查通过。
核实官方TRL0.24源码的β=0分支；证据保存在`analysis/rl_extra_implementation_20260909/`。
当前会话无训练服务器连接或CUDA设备，尚未启动正式训练，也没有F组效果结果。
